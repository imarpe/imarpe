...
Esta función principal es usada para dos tipos de clases de datos: fishery y cpue. La primera clase, `fishery`, contiene variables pesqueras que en este caso incluye a los desembarques (landing) y al esfuerzo (effort). La segunda clase de datos, `cpue`, incluye a la captura por unidad de esfuerzo (cpue).

`imarpe` incluye una base de datos (`fisheryData`) para ejemplificar el uso de esta función. Sin embargo, toda base de datos debe poseer información sobre el tiempo (año, mes y dia), sobre la especie, sobre el tipo de flota y puerto, la información de posición (que es opcional), e imprescindible la información de la captura (o desembarque), así como las variables que serán utilizadas en el esfuerzo pesquero. Se puede utilizar la ayuda de `fisheryData` para conocer un poco más de su contenido. 
...
Luego se utilizará la `getFishingData` para analizar los desembarques de la base de datos bringada. La ayuda de esta función puede ser visualizada para una mejor comprensión de sus parámetros.
...
# Usar getFishingData con la base de datos ejemplo (fisheryData)
?getFishingData
desembarque = getFishingData(file = fisheryData, type = "fisheryInfo", varType = "landing",
                             sp = "caballa")
```


`getFishingData` lee información pesquera para obtener i) un data.frame (o marco de datos) y ii) una lista con las principales características de dicho data.frame. 
