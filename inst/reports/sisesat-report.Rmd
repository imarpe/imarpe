---
title: "Reporte semanal de la pesquería del Stock Norte Centro de anchoveta (Temporada 2016 - I)"
#author: "(DGIRP - AFDPERP)"
...
out.semana  = estimaSemana2(inicio.temp = inicio_temp, fin.temp = fin_temp)
...
mapa.peru.simple(xlim = c(wf, -70), add = TRUE, labelsxy = F, axis4L = F,perfil = TRUE)
title("Áreas de pesca")

for(i in 1:length(space)){
  lines(shoreline$V1 + space[i], shoreline$V2, col = "gray")
...
# map hotspots
mapa.peru(xlim = c(wf, -70), labelsxy = F, axis4L = F,perfil = TRUE)
for(i in 1:length(space)){
  lines(shoreline$V1 + space[i], shoreline$V2, col = "gray")
  text(x = -72 + space[i], y = -19, labels = rev(labelSEMANA)[i], cex = 0.7)
}
