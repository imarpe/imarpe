...
monthTable = rbind(monthTable, apply(monthTable, 2, mean, na.rm = TRUE))
rownames(monthTable) = c(rowNamesTable, "Total")
monthTable = cbind(monthTable, apply(monthTable, 1, mean, na.rm = TRUE))
colnames(monthTable) = c(colNamesTable, "Total")

monthTable = round(x = monthTable, digits = 3)

pander::pander(monthTable, align = "c", keep.line.breaks = TRUE)
```

# Cpue por tipo de flota
```{r, echo=FALSE, message=FALSE}
fleeTable = x$fleeTable
rowNamesTable = rownames(fleeTable)
colNamesTable = colnames(fleeTable)

fleeTable = rbind(fleeTable, apply(fleeTable, 2, mean, na.rm = TRUE))
rownames(fleeTable) = c(rowNamesTable, "Total")

fleeTable = cbind(fleeTable, apply(fleeTable, 1, mean, na.rm = TRUE))
colnames(fleeTable) = c(colNamesTable, "Total")

fleeTable = round(x = fleeTable, digits = 3)

pander::pander(fleeTable, align = "c", keep.line.breaks = TRUE)
```
\pagebreak

# Cpue diario, mensual y anual
...
if(dim(portTable)[2] == 1){
  ...
  portTable = rbind(portTable, apply(portTable, 2, mean, na.rm = TRUE))
  ...
  if(dim(portTable)[2] == 1){
    ...
    portTable = rbind(portTable, apply(portTable, 2, mean, na.rm = TRUE))
    rownames(portTable) = c(rowNamesTable, "Total")
    portTable = cbind(portTable, apply(portTable, 1, mean, na.rm = TRUE))
    colnames(portTable) = c(colNamesTable, "Total")
    
  }
}
portTable = round(x = portTable, digits = 3)

pander::pander(portTable, align = "c", keep.line.breaks = TRUE)
```

# Cpue acumulado en el dominio peruano
...
# Cpue acumulado en la región Norte-Centro
...
# Cpue acumulado en la región Sur
```{r, echo=FALSE, message=FALSE, fig.height=5.5, fig.width=9.5, dpi=72}
