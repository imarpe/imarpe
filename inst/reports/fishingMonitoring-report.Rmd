...
for(i in seq(length(allIndex) + 1)){
  ...
  tempJuvs <- c(sum(tempJuvs), 
                sum(tempJuvs)/sum(tempData)*100, 
                sum(x$a*x$allMarks[x$allMarks < 12]^x$b*tempJuvs), 
                sum(x$a*x$allMarks[x$allMarks < 12]^x$b*tempJuvs)/sum(x$a*x$allMarks^x$b*tempData)*100)
