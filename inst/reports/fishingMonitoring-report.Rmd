...
# Tabla diaria
index <- match(as.Date(x$endDate, format = "%Y-%m-%d"), x$allDates)
...
output <- cbind(x$allMarks, x$surveyVector, rowSums(x$catchData), x$outputByDay[,ncol(x$outputByDay)])
...
for(i in seq(2, ncol(output))){
  ...
  axis(side = 2, at = seq(ylim[ifelse(i == ncol(output), 1, 3)], ylim[2], ylim[3]), las = 2)
  
  ylabs2 <- seq(ylim2[1], ylim2[2], ylim2[3])/1e6
  axis(side = 4, at = seq(ylim[ifelse(i == ncol(output), 1, 3)], ylim[2], length.out = length(ylabs2)), 
       labels = ylabs2, las = 2)
