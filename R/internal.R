...
.trimData = function(x, start, end) {
  ...
  if(is.null(start) & is.null(end)){
    ...
    vectorDates = as.Date(as.character(paste(x$year,"-",match(months, unique(months)), "-",x$day,sep="")),format="%Y-%m-%d")
  ...
  datesVector = paste(x$year, "-", match(months, tolower(month.abb)), "-", x$day, sep = "")
